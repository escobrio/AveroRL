...
def plot_episode(observations, infos, actions, rewards):
    ...
    for i in range(19, 22, 1):
        axs[3,0].plot(states[:, i], label=f"fanspeeds_{i-19}", color=basic_colors[i-19])
        axs[3,0].plot(actions[:, i-19], label=f"fanspeeds_setpoints{i-19}", linestyle='--', color=basic_colors[i-19])

...
def BT_plot(observations, infos, actions, rewards):
    ...
    # plt.title("Linear Velocity reference tracking", loc='center')
    # plt.ylabel(r"${}_{B}{v} \, [\mathrm{m/s}]$")
    # plt.ylim(bottom=-1, top=2.5)
    # plt.xlabel("time [s]")
    # plt.axhline(0, color='black', alpha=0.2)
    # time = np.arange(len(states)) * 0.01
    # plt.plot(time, states[:, 7], color=colors[0], label=r"${}_{B} {v_x}$")
    # plt.plot(time, states[:, 8], color=colors[1], label=r"${}_{B} {v_y}$")
    # plt.plot(time, states[:, 9], color=colors[2], label=r"${}_{B} {v_z}$")
    # # plt.plot(time, states[:, 7], '.', markersize=1, color=colors[0])
    # # plt.plot(time, states[:, 8], '.', markersize=1, color=colors[1])
    # # plt.plot(time, states[:, 9], '.', markersize=1, color=colors[2])
    # plt.plot(time, vel_ref[:, 0], color=colors[0], alpha=0.3)
    # plt.plot(time, vel_ref[:, 1], color=colors[1], alpha=0.3)
    # plt.plot(time, vel_ref[:, 2], color=colors[2], label=r"${}_{B} v_z^{\ \mathrm{ref}}$", alpha=0.3)
    ...
    plt.savefig("data/eval/orientation_tracking.pdf", format='pdf', bbox_inches="tight")
    plt.savefig("data/eval/orientation_tracking.png", format='png', dpi=600, bbox_inches="tight")
